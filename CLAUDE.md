...
- `~/.bash_profile`/`~/.zshrc` only run for interactive shells, so a tool that spawns a shell non-interactively (or bash as a non-login interactive shell) never sees `~/.shell.d`. `install.sh` also points `~/.zshenv` (read by **every** zsh process) and `~/.bashrc` (read by non-login interactive bash) directly at a `~/.shell.d/*` sourcing loop — via `install_shell_env_loader`, the same inject-at-top/create-if-missing pattern as `install_shell_loader` — without pulling in the full `dotfiles/bash_profile`/`dotfiles/zshrc` chain (completion/prompt/alias setup would be wasteful or wrong outside a real interactive session).
